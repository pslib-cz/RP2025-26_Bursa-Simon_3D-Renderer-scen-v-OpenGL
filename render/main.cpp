...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        Rectangle saveBtnRect = { (float)sw - 270, 10, 60, 25 };
        Rectangle loadBtnRect = { (float)sw - 205, 10, 60, 25 };
        Rectangle returnBtnRect = { (float)sw - 140, 10, 120, 25 };
        ...
        if (currentScene == SCENE_WHITE) {
            ...
            if (editingFilename) {
                ...
                while ((key = GetCharPressed()) > 0) {
                    if (key >= 32 && fileLen < 255) {
                        saveFile[fileLen++] = (char)key;
                        saveFile[fileLen] = '\0';
                    }
                }
        ...
        if (currentScene == SCENE_WHITE) {
            ...
            if (editingFilename && ((int)(GetTime() * 2) % 2 == 0)) {
                int cx = (int)fileInputRect.x + 3 + MeasureText(saveFile, 9);
                if (cx < (int)(fileInputRect.x + fileInputRect.width - 2))
                    DrawLine(cx, (int)fileInputRect.y + 2, cx, (int)fileInputRect.y + 15,
                        { 0, 200, 255, 255 });
            }

            GuiSetState(brushMode ? STATE_PRESSED : STATE_NORMAL);
            if (GuiButton(brushBtnRect, "#22#")) {
                brushMode = !brushMode;
                eraserMode = false;
                selectionMode = false;
                ClearSelection(&selGrid, &sel);
            ...
            if (GuiButton(eraserBtnRect, "#143#")) {
            ...
            if (GuiButton(selectBtnRect, "#32#")) {
                selectionMode = !selectionMode;
                brushMode = false;
                eraserMode = false;
                if (!selectionMode) ClearSelection(&selGrid, &sel);
            }
            GuiSetState(STATE_NORMAL);


            if (selectionMode && sel.count > 0) {
            ...
            if (GuiButton({ cx + 10,  (float)sh - 40, 100, 30 }, "WALK (3D)")) currentScene = SCENE_BLUE;
            ...
            if (menu.active) {
                ...
                if (menu.editMode == 1) {
                    ...
                    float h = (menu.editMode == 0) ? 145.0f : (menu.editMode == 2 ? 140.0f : 80.0f);
                    DrawRectangleRec({ mx, my, 130, h }, { 45, 45, 45, 255 });
                    DrawRectangleLinesEx({ mx, my, 130, h }, 1, LIGHTGRAY);
                    if (menu.editMode == 0) {
            ...
            if (GuiButton({ cx2 + 10,  (float)sh - 40, 100, 30 }, "WALK (3D)")) currentScene = SCENE_BLUE;
        }
